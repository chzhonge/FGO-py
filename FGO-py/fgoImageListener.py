import os,cv2,numpy,platform,time
...
if platform.system()=='Windows':
    ...
    class DirListener:
        def __init__(self,dir):
            ...
            def f():
                while True:
                    self.add(win32file.ReadDirectoryChangesW(self.hDir,0x4000,False,win32con.FILE_NOTIFY_CHANGE_FILE_NAME|win32con.FILE_NOTIFY_CHANGE_LAST_WRITE,None,None))
                    if len(self.msg)>1000:self.msg=[]
...
class ImageListener(dict):
    def __init__(self,path,ends='.png'):
        ...
        self.ends=ends
        data = {}
        for file in [file for file in os.listdir(self.path) if file.lower().endswith(ends.lower())]:
            name = file[:-len(ends)]
            if (val := self._safeRead(self.path+file)) is not None:
                data[name] = val
        super().__init__(data)
        self.listener=DirListener(self.path)
    def _safeRead(self,path,retries=3):
    ...
    def flush(self):
        ...
        oldName=None
        def onCreated(name):
            if(val:=self._safeRead(self.path+name+self.ends))is not None:self[name]=val
        def onDeleted(name):
        ...
        def onRenamedFrom(name):
            nonlocal oldName
            if oldName is not None and oldName in self:del self[oldName]
            oldName=name
        def onRenamedTo(name):
            if lastAction==4 and oldName in self:
                self[name]=self[oldName]
            elif(val:=self._safeRead(self.path+name+self.ends))is not None:
                self[name]=val
        for action,name in((action,file[:-len(self.ends)])for action,file in self.listener.get()if file.lower().endswith(self.ends.lower())):
            try:
